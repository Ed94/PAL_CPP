...
ForceInline p<void const>
Search(p<void const> data, u8 value, sw size)
{
	p<u8 const> byteData = p<u8 const>(data);

	while	((rcast<uiPtr>(byteData) & (sizeof(uw) - 1)) 
			&& size 
			&& dref byteData != value)
	{
		byteData++;
	...
	if (size && dref byteData != value) 
	{
		p<sw const> word;

		sw k = ones * value;

		word = rcast< p<sw const>>(byteData);

		while (size >= sizeof(sw) && !HasZero(dref word ^ k)) 
		{
			word++;
			size -= sizeof(sw);
		}

		byteData = rcast< p<u8 const>>(word);

		while (size && dref byteData != value) 
		{
			byteData++;
			size--;
		}
	...
	return size ? rcast< p<void const>>(byteData) : nullptr;
}
